...
export function RiskRegisterProvider({ children }: { children: ReactNode }) { const [records, setRecords] = useState<RiskRegisterRecord[]>([]); const addRecord = (record: RiskRegisterRecord) => setRecords((current) => appendRiskRecord(current, record)); return <RegisterContext.Provider value={{ records, addRecord }}>{children}</RegisterContext.Provider>; }
